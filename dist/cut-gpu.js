 ...
 */
import { acquireContext, releaseContext, isContextReady, getContext, getContextGeneration, registerProgram, hasProgram, getProgram, useProgram, assertOutputSize, bindRenderTarget, transferBitmap, runExclusive, isImageBitmap, createTexture, deleteTexture, readPixels, readPixelsAsync, draw, clear, } from './gpu-context.js';
import { CUT_PASS0_VERTEX, CUT_PASS0_FRAGMENT, CUT_PASS1_VERTEX, CUT_PASS1_FRAGMENT, CUT_PASS2_VERTEX, CUT_PASS2_FRAGMENT, } from './cut-shaders.js';
const P0 = 'cut3_p0';
const P1 = 'cut3_p1';
const P2 = 'cut3_p2';
...
export function resolveCutOptions(options) {
...
export class CutGpuRenderer {
    ...
    ensureResources() {
        ...
        if (this.contextGen === gen && this.inputTexture)
            return;
        if (!hasProgram(P0))
            registerProgram(P0, CUT_PASS0_VERTEX, CUT_PASS0_FRAGMENT, P0_UNIFORMS);
        if (!hasProgram(P1))
            registerProgram(P1, CUT_PASS1_VERTEX, CUT_PASS1_FRAGMENT, P1_UNIFORMS);
        if (!hasProgram(P2))
            registerProgram(P2, CUT_PASS2_VERTEX, CUT_PASS2_FRAGMENT, P2_UNIFORMS);
        const { gl } = getContext();
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (!fbo)
                throw new Error('Failed to create CUT3 pass framebuffer');
            this.passFbos[i] = fbo;
        }
    ...
    ensurePassTargets(width, height) {
        if (this.passSize.width === width && this.passSize.height === height)
            return;
        const { gl } = getContext();
        for (let i = 0; i < 2; i++) {
...
//# sourceMappingURL=cut-gpu.js.map