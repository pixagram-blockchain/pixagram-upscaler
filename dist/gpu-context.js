...
export function isImageBitmap(source) {
...
export function releaseContext() {
    ...
    if (contextRefCount <= 0 && sharedContext) {
        ...
        if (sharedContext.fboTexture)
            gl.deleteTexture(sharedContext.fboTexture);
        if (sharedContext.fbo)
            gl.deleteFramebuffer(sharedContext.fbo);
 ...
 */
export function getContextGeneration() {
...
export function getContext() {
    ...
    if (sharedContext.gl.isContextLost()) {
        throw new Error('WebGL context lost. Rendering will recover automatically once the ' +
 ...
 */
export function getMaxOutputDimension() {
    const { limits } = getContext();
    return Math.min(limits.maxTextureSize, limits.maxRenderbufferSize, limits.maxViewportWidth, limits.maxViewportHeight);
}
/**
 ...
 */
export function assertOutputSize(width, height) {
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
        throw new Error(`Invalid output size ${width}x${height}`);
    }
    const max = getMaxOutputDimension();
    if (width > max || height > max) {
        throw new Error(`Output size ${width}x${height} exceeds this GPU's limit of ${max}px per side. ` +
 ...
 */
export function registerProgram(id, vertexSource, fragmentSource, uniformNames) {
...
export function getProgram(id) {
    ...
    if (!info)
        throw new Error(`Program not registered: ${id}`);
    return info;
}
/**
 ...
 */
export function unregisterProgram(id) {
    const info = programs.get(id);
 ...
 */
export function bindRenderTarget(width, height, target) {
    const ctx = getContext();
    const { gl } = ctx;
    if (target === 'texture') {
        if (!ctx.fbo) {
 ...
 */
export function transferBitmap() {
    const ctx = getContext();
 ...
 */
export function uploadToSharedTexture(data, width, height, filter = 'nearest') {
    const { gl } = getContext();
 ...
 */
export function createTexture(filter = 'nearest') {
    const { gl } = getContext();
 ...
 */
export function readPixels(width, height, out) {
    const { gl } = getContext();
 ...
 */
function clientWaitAsync(gl, sync, intervalMs = 0) {
 ...
 */
export async function readPixelsAsync(width, height, out) {
    const ctx = getContext();
    const { gl } = ctx;
    const size = width * height * 4;
    if (!ctx.packBuffer) {
        ...
        if (!ctx.packBuffer)
            throw new Error('Failed to create pack buffer');
    }
